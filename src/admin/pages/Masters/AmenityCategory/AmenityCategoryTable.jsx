...
import AmenityCategoryRow from "../AmenityCategory/AmenityCategoryRow";
import MasterLayout from "../../../components/layout/MasterLayout";
import Pagination from "../../../components/Pagination";
import { getAllAmenityCategory } from "../../../../api";

export default function AmenityCategoryTable() {
  ...
  const fetchAmenityCategory = async () => {
    ...
    try {
      ...
      const result = await getAllAmenityCategory(offset, itemsPerPage, search);

      if (result?.status && Array.isArray(result.data)) {
        setAmenityCategory(result.data);
      ...
      console.error("Error fetching AmenityCategory:", error);
      setAmenityCategory([]);
      setTotalCount(0);
    } finally { 
      setLoading(false);
  ...
  // --- Fixed Search Filter ---
const filteredAmenityCategory = AmenityCategory.filter((p) =>  
  p.amenitie_category_name?.toLowerCase().includes(search.toLowerCase())  
);

// --- Pagination ---
const totalPages = Math.ceil(totalCount / itemsPerPage) || 1;  
const currentAmenityCategory = filteredAmenityCategory; // ✅ now maps filtered results
        ...
        <div className="px-5 mb-5">
          <div className="card">
            <div className="card-body p-5">
              {loading ? (
                <p>Loading...</p>
              ) : filteredAmenityCategory.length === 0 ? (
                <p>No AmenityCategory found</p>
              ) : (
                <div className="text-nowrap theme-scrollbar-horizontal">
                  <table className="theme-table">
                    <thead>
                      <tr>
                        <th>ID</th>
                        <th>Amenity Category Name</th>
                        <th>Date</th>
                        <th>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {currentAmenityCategory.map((p) => (
                        <AmenityCategoryRow
                          key={p.id || p.project_id}
                          project={p}
                          refreshDate={fetchAmenityCategory}
